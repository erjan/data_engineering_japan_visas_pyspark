...
spark = SparkSession.builder.appName("me doing e2d spark dataset analysis").getOrCreate()


df = spark.read.csv('input/visa_number_in_japan.csv', header=True, inferSchema=True)

newcolnames = [colname.replace(' ', '_').replace('/', '').replace('.', '').replace(',','') for colname in df.columns]
...
#top 10 countries with most visa issued
...
select country, sum(number_of_issued_numerical) visa_issued
                       ...
                       and year = 2017
                       group by country order by visa_issued desc limit 10""")
...
# --------------------display the output on the WORLD MAP
